...
describe("FormField", function() {
  it("renders the base component", function() {
  ...
  it('contains a text input when type is text', function() {
    const wrapper = shallow(<FormField type="text" />);
    expect(wrapper.find('input[type="text"]').length).toEqual(1);
  });

  it('contains a textarea when type is textarea', function() {
    const wrapper = shallow(<FormField type="textarea" />);
    expect(wrapper.find('textarea').length).toEqual(1);
  });

  it('renders an error when present', function() {
